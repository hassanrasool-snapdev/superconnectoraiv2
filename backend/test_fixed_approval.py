...
async def test_fixed_approval():
    """Test the fixed approval workflow"""
    print("Testing fixed approval workflow...")
    
    # Emily Davis's request ID
    ...
    try:
        ...
        if login_response.status_code == 200:
            ...
            if approval_response.status_code == 200:
                ...
                print(f"Temporary password: {response_data['user']['otp']}")
                print(f"Email subject: {response_data['email_template']['subject']}")
                print("✅ Email template generated successfully!")
            else:
                print(f"❌ Approval failed with status: {approval_response.status_code}")
                try:
...
if __name__ == "__main__":
    asyncio.run(test_fixed_approval())