        ...
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                connection = {
                    ...
                    "about": row.get('Description/0', ''),
                    "city": row.get('City', ''),
                    "country": row.get('Country', ''),
                    "companyName": row.get('Company', '') or row.get('CompanyName', ''),
                    "title": row.get('Title', ''),
                    "experiences": f"Works at {row.get('Company', '')} as {row.get('Title', '')}",
                    "education": "",
                    "skills": "",
                    "linkedin_url": row.get('LinkedinUrl', ''),
                    "followerCount": int(row.get('Followers', '0').replace(',', '').replace('K', '000').replace('M', '000000')) if row.get('Followers') else 0,
                    "connectionsCount": 0,
                    "isOpenToWork": False,
                    "isHiring": False,
                    "is_company_owner": "founder" in row.get('Title', '').lower() or "ceo" in row.get('Title', '').lower(),
